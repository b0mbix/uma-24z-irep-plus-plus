...
class Ripper:
    def __init__(self, max_iterations: int = 20, max_conditions_in_rule: int = 5, prune_percentage: float = 1/3, random_state: int = None, verbose_level: int = 0):
        ...
        self._logger.info(f"Initialized Ripper with max_iterations={self._max_iterations}, max_conditions_in_rule={self._max_conditions_in_rule}, test_percentage={self._test_percentage:.2f}")
    ...
    def _prune_rule(self, rule, X, y):
        ...
        for _ in range(len(rule)):
            pruned_rule = rule[:-1]
            if self._prune_metric(pruned_rule, X, y) >= self._prune_metric(rule, X, y):
                rule = pruned_rule
            else:
                return rule
        return rule

    def _prune_metric(self, rule, x, y):
