...
class IRepPlusPlus:
    ...
    def _prune_rule(self, rule, X, y):
        ...
        for _ in range(len(rule)):
            pruned_rule = rule[:-1]
            if self._prune_information_gain(rule, pruned_rule, X, y) <= 0:
                rule = pruned_rule
            else:
                return rule
        return rule
    ...
    def _prune_information_gain(self, rule, pruned_rule, x, y):
        ...
        p0, n0 = sum(y[pruned_covered] == 1), sum(y[pruned_covered] == 0)
        p, n = sum(y[original_covered] == 1), sum(y[original_covered] == 0)
        return self._calculate_information_gain(p, n, p0, n0)
